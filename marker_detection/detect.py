...
#    print("model_points:",model_points,"image_points",image_points,"camera_matrix:",camera_matrix)
    ...
    trans_res=(round(translation_vector[0][0],2),round(translation_vector[1][0],2),round(translation_vector[2][0],2))
...
#    gray = cv2.medianBlur(gray,3)
    ...
    # Otsu's thresholding
    ret2,gray_edge = cv2.threshold(gray.copy(),175,255,cv2.THRESH_BINARY)#+cv2.THRESH_OTSU
...
#        print("sorted_curve:",sorted_curve)
        ...
        warped_img = cv2.warpPerspective(gray_edge, persp_transf, (warped_size, warped_size))
...
#        cv2.waitKey(0)
        ...
        ret2,wraped_bin = cv2.threshold(warped_bin.copy(),190,255,cv2.THRESH_BINARY)#
